...
5. **選択肢シャッフル**
   正解位置をランダム化し、解説文中のA/B/C/D参照も新しい記号に書き換える（`remapChoiceLetters`）。
...
- **max_tokens**: 生成系呼び出しは用途ごとに異なる（読解生成 32,000 / 語彙生成 12,000 / 読解アノテーション 20,000 / 語彙アノテーション 8,000。詳細は `lib/claude.ts` 内の各Anthropic API呼び出しを参照）
...
- 選択肢シャッフル後の記号（A/B/C/D）と解説文中の参照の同期チェック（`verifyChoiceLabelConsistency`）
- 解説と設問のマッピング整合性チェック（`validateExplanationMapping`）

**プロンプト側（意味的チェック）**
- 誤答の作り方（技法の配分、本文由来の要素の使い方）
- パッセージの論説構成の質、パラフレーズの抽象度
- 語彙アノテーションの紛らわしい類義語ペア（confusingPairs）の妥当性

機械的チェックのうち一部（語数バランス、正解最長、極端語）は警告ログのみでリトライには乗せず、設問単位の重大な違反（空所欠落・語プール逸脱等）のみが個別リトライの対象になる。

## ルールバージョン

現在の生成ルールは **v5.1**（サブバージョン: v5.1.1 語彙固定グループ化、v5.1.3 アノテーション分離・35語超過エスカレーション）。各バージョンでの変更点・解決した課題は [CHANGELOG.md](./CHANGELOG.md) を参照。
