...
- **語彙生成モデル**（`generateVocabOnly`・`retryVocabQuestion`）: 環境変数 `GENERATION_MODEL`（**未設定＝コード側デフォルトの`claude-opus-5`、v5.15**。Haiku 4.5・Sonnet 5・Opus 5の3モデル比較の結果、安定性・品質を優先してOpus 5に変更。経緯は下記「モデル選定の経緯」を参照）
...
2026年7月にSonnet 5へ切り替え、9月にHaiku 4.5へ戻し、同月中に読解のみ再度Sonnet 5へ戻し、語彙はOpus 5へ切り替えるという経緯を辿った。
...
5. **2026-09-25（v5.15）**: 語彙生成を同一単語セット（3セット）でHaiku 4.5・Sonnet 5・Opus 5それぞれ3回ずつ比較。Haiku 4.5は初回下書きが15問全問「空所`____`なし＋正解語の埋め込み」でリトライを要し、リトライ後も15問中2問が空所なしのまま残った。Sonnet 5は3回中2回がadaptive thinkingで`max_tokens`（16,000）を使い切りJSON未完成で生成自体が失敗した。Opus 5は3回とも全問一発合格・欠陥ゼロ。1日分（5問・リトライ込み）の実コストはHaiku 約$0.071／Sonnet 約$0.155／Opus 約$0.186（Haikuの約2.6倍）だが、安定性・品質を優先して`GENERATION_MODEL`のデフォルトを`claude-opus-5`に変更した。語彙は読解（Sonnet 5、150〜270秒）と並列実行されるため、Opus 5の所要時間（約90〜110秒）は全体の所要時間を延ばさない
...
| `GENERATION_MODEL` | 語彙生成に使うモデルID | 未設定時は `claude-opus-5` にフォールバック（v5.15。旧: `claude-haiku-4-5`） |
...
- 語彙: 空所の存在・個数、正解語の露出（v5.15: 単語の語頭で正解語・活用形/派生形の語幹と一致するもののみを露出とみなす。旧実装の先頭5文字の部分一致は"intervene"→"international"等の誤検知があった）、選択肢4つの重複なし、指定語プール外の語がないか、設問間の20語重複なし（`validateOneVocabQuestion` / `validateVocabQuestions`）
